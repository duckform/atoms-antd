...
export const quick: QuickTransformer = {
  ...
  transform: (node) => {
    ...
    const done = arrayItems?.children?.find?.(
      (x) => getNodeProps(x, "x-component") === "ProArrayTable.Column",
    );
    if (done) return node;
    ...
    const actions = new TreeNode(
      {
        ...
        children: ["MoveUp", "MoveDown", "Remove"].map((Action) => {
          return {
            ...
            props: {
              type: "void",
              "x-component": `ProArrayTable.${Action}`,
            },
          };
        }),
      },
  ...
  reset: (node) => {
    // if (!ProArrayTable.match(node)) return node;
    const arrayItems = node.children?.[0];
    ...
    const unwrapColumns = items.reduce<TreeNode[]>((list, column) => {
      ...
      // 约定 ProArrayTable. 开头的组件都是 ProArrayTable 的辅助组件
      const clean = properties.filter(
        (n) => !/^ProArrayTable\./.test(getNodeProps(n, "x-component")),
      );
      list.push(...clean);
      return list;
    }, []);
